...
# based on examples/asynchronous_consumer_example.py
# in https://github.com/pika/pika
# (commit 351f272f8fce585d986ecf293cdf476aeb9a73c7)
# Adapted by Ulrich Germann
...
class AMQP_Worker(object):
    def __init__(self, amqp_url, queue, exchange='', exchange_type='topic',
                 routing_key='', prefetch_count=1):
    ...
    def on_connection_closed(self, connection, errcode, reason):
        ...
        """
        logger.info("Connection closed [%r] %s"%(errcode, reason))
        self._channel = None
        if self._closing:
    ...
    def on_queue_declareok(self, method_frame, userdata):
        ...
        if len(self._exchange):
            logger.info('Binding %s to %s with %s',
                        self._exchange, queue_name, self._routing_key)
            ...
            self.on_bindok(queue_name, userdata=queue_name)
